...
loadProgram() {
    ...
    insMEM[5] = 0x10810000; //  LOAD R1, 0(R4)         element2 = MEM[index2]
    insMEM[10] = 0x14420001; // ADDI R2, R2, 1         index1++
    insMEM[15] = 0x10400000; // LOAD R0, 0(R2)         element1 = MEM[index1]
    insMEM[20] = 0x04012800; // ADD R5, R0, R1         temp = element1 + element2
    insMEM[25] = 0x18C50000; // STORE R5, 0(R6)        MEM[index3] = temp
    insMEM[30] = 0x14C60001; // ADDI R6, R6, 1         index3++
    insMEM[35] = 0x1508FFFF; // ADDI R8, R8, -1        count--
    insMEM[36] = 0x2500FFDC; // BNEZ R8 -36            if (count != 0) goto top
    insMEM[40] = 0x00000000; // NOP 
    insMEM[45] = 0x28000000; //HALT
}
