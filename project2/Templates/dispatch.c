...
int do_dispatch() {
  ...
  for (i= next; i < next + NUM_RESERVATION_STATIONS; i=i+1) {
    if (RS[i % NUM_RESERVATION_STATIONS].free == TRUE)        // Does not hold a valid instruction
      continue;
    if (RS[i % NUM_RESERVATION_STATIONS].busy == TRUE)     // Instruction has been dispatched
      continue;
